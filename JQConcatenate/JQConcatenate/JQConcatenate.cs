...
public struct JQConcatenate : IBinarySerialize
{
    ...
    public void Accumulate(SqlString Value, SqlString Delimiter)
    {
        if (!Value.IsNull)
        {
            ...
            SqlString myValue = new SqlString(Value.Value.Replace("\u0000", ""));
            if (myValue.Value.Length > 0)
            {
                ...
                _accumulator.Append(myValue.Value);
                if (myValue.IsNull == false) this.IsNull = false;
    ...
    public void Merge(JQConcatenate Group)
    {
        /// add the delimiter between strings
        if (_accumulator.Length > 0
            & Group._accumulator.Length > 0)
